...
export class TeacherApiService extends BaseApiService {
    ...
    async getLessons(): Promise<Lesson[]> {
        ...
        return lessons.map(lesson => ({
            ...
            salesCount: lesson.salesCount ? Number(lesson.salesCount) : 0
        }));
    }

    // Create new lesson
    async createLesson(
        lessonData: Omit<Lesson, 'id' | 'teacherId' | 'teacherName' | 'createdAt'> | FormData
    ): Promise<Lesson> {
        // --- THIS IS THE FIX ---
        // It now points to '/lessons' instead of '/teacher/lessons'
    ...
    async updateLesson(
        id: string,
        lessonData: Partial<Lesson> | FormData
    ): Promise<Lesson> {
        // --- THIS IS THE FIX ---
        // It now points to '/lessons/:id' instead of '/teacher/lessons/:id'
    ...
    async deleteLesson(id: string): Promise<void> {
        // This one is correct because it's a teacher-specific action
        return this.delete<void>(`${API_ENDPOINTS.TEACHER.LESSONS}/${id}`);
    }

    // Get students who purchased a specific lesson
    ...
    async getSalesStatistics() {
        ...
        console.log('Raw sales stats from API:', data);
        return {
